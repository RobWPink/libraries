...
public:
   ...
   * @retval false Failed
   */
  bool setServiceProvider(const char *apn);
  ...
  */ 
   ...
   * @retval false Failed
   */
  bool restart(uint8_t powerPin, uint8_t resetPin);
   ...
   * @retval false Failed
   */
  bool GNSS_Toggle(bool setting);
  
    /**
   * @fn GNSS_AT_Set
   * @brief Set GNSS to output NMEA to AT Port
   * @return bool type, indicating success
   * @retval true Success 
   * @retval false Failed
   */
  bool GNSS_AT_Set(bool enableAT, int packetCount);
   ...
   * @retval false Failed
   */
  bool MQTT_Config(const char* tag, const char* val);
   ...
   * @retval false Failed
   */
  bool MQTT_ConfigUrl(char* url, char* port);
   ...
   * @retval false Failed
   */
  bool MQTT_ConnectStatus(uint32_t timeout);
  
  /**
   * @fn getTimestamp
   * @brief Get timestamp from cellular network
   * @return bool type, inidicating if the timestamp is retrieved
   * @retval true Success 
   * @retval false Failed
   */
  ...
  /**
   * @fn getTime
   * @brief Get time from cellular network
   * @return bool type, inidicating if the time is retrieved
   * @retval true Success 
   * @retval false Failed
   */
  bool getTime(tm* timeBuffer);
  
  int checkNTPStatus();
  bool NTP_Configure(const char* ntpUrl, int ntpTimezone, int cid, int ntpMode);
  bool bearerProfile_Open(int profile);
  bool bearerProfile_Configure(int profile, const char* parameter, const char* value);
  
  
  /**
   * @fn ToggleWebApp
   * @brief Enable MQTT Application
   * @return bool type, indicating if toggle was succesful
   * @retval true Success 
   * @retval false Failed
   */
  bool ToggleWebApp(uint8_t setting, const char *apn);
  
  bool MQTT_Subscribe(const char* topic, int QoS);
  bool MQTT_Unsubscribe(const char* topic);
  int  MQTT_SubscribeParse(char* keyBuffer, char* valBuffer, uint16_t timeout);
  ...
  bool MQTT_PublishAsPbuf(const char* topic);
